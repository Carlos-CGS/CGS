...
function checkScrollPosition() {
  ...
  var ctaContact = document.querySelector('.cta-contact');
  var scrolled = document.body.scrollTop > 20 || document.documentElement.scrollTop > 20;

  if (btnHome) btnHome.classList.toggle('visible', scrolled);
  if (ctaContact) ctaContact.classList.toggle('visible', scrolled);
}

window.onscroll = checkScrollPosition;
document.addEventListener('DOMContentLoaded', checkScrollPosition);

// ===== Ano final automatico no rodape (mantem "2024" fixo, so o ano final acompanha o relogio) =====
...
// ===== Trailer Intro =====
(function () {
  const INTRO_SEEN_KEY = 'cgs_intro_seen';
  ...
  try {
    alreadySeen = localStorage.getItem(INTRO_SEEN_KEY) === 'true';
  } catch (e) {}

  // Visitante que ja viu a intro (ou prefere menos movimento) vai direto ao conteudo.
  if (prefersReduce || alreadySeen) {
...
// ===== Email Links: mailto + fallback (aplica a #email-link e ao CTA "Fale comigo") =====
document.addEventListener('DOMContentLoaded', function () {
  const emailLinks = document.querySelectorAll('.mailto-fallback-link, #email-link');
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (subtitle) {
      ...
      badge.textContent = 'Disponível para projetos';
...
(function () {
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const featuredItems = [
      ...
      'Bacharelado em Tecnologia da Informação - UNIVESP - (2023-2026)',
    ...
    featured.setAttribute('aria-label', 'Formação e certificações em destaque');

    const title = document.createElement('h3');
    title.textContent = 'Formação e Certificações em destaque';
...
(function () {
  const projectMap = {
    uniguardião: {
      status: 'Em evolução',
      stack: ['Full Stack', 'PHP', 'MySQL'],
      case: {
        problem: 'Capacitação de colaboradores dispersa e sem acompanhamento',
        solution: 'Plataforma com trilhas, cursos, avaliações e certificados',
        result: 'Treinamento corporativo centralizado e mensurável'
      }
    },
    worldwar: {
      status: 'Em evolução',
    ...
    appgestao: {
      ...
      case: {
        problem: 'Gestão manual de equipe',
        solution: 'Plataforma web com KPIs e relatórios',
    ...
    kidsrunner: {
      ...
      case: {
        ...
        solution: 'Runner com gestos e pontuação',
        result: 'Jogo jogável com feedback em tempo real'
      }
    },
    'controle de acesso': {
      status: 'Concluído',
      ...
      case: {
        problem: 'Controle local sem padronização',
        solution: 'Aplicativo desktop para registros',
        result: 'Fluxo local mais confiável'
      }
    },
    'j.a.r.v.i.s': {
      status: 'Concluído',
      ...
      case: {
        ...
        solution: 'Assistente por voz com automações',
        result: 'Execução rápida de comandos cotidianos'
      }
    },
    'world war - game': {
      status: 'Concluído',
      ...
      case: {
        problem: 'Demonstrar lógica de batalha por cartas',
        solution: 'Motor de rodada com regras de combate',
        result: 'Gameplay estável para web'
      }
    },
    avatar: {
      status: 'Concluído',
      ...
      case: {
        problem: 'Busca técnica com baixa contextualização',
        solution: 'Avatar + descrição por tecnologia',
        result: 'Experiência de consulta mais didática'
      }
    },
    'desafio do heroi': {
      status: 'Concluído',
      ...
      case: {
        problem: 'Classificação de nível sem padrão visual',
        solution: 'Formulário com regra de XP',
        result: 'Feedback imediato de nível do usuário'
      }
    },
    'projeto pokedex': {
      status: 'Concluído',
      ...
      case: {
        ...
        solution: 'Pokédex com detalhamento e filtros visuais',
        result: 'Consulta rápida de catálogo'
      }
    },
    codeverse: {
      status: 'Concluído',
      ...
      case: {
        ...
        result: 'Conteúdo técnico mais acessível'
      }
    }
  };
...
(function () {
  ...
  // Camadas fotograficas do hero: foto ao fundo (mais lenta) + ornamento em primeiro
  // plano (mais rapido, mais "proximo" da cena), reforcando o parallax.
  gsap.to('.hero-photo-layer', {
    yPercent: heroPhotoShift,
    ease: 'none',
    scrollTrigger: {
      trigger: hero,
      start: 'top top',
  ...
  gsap.to('.hero-ornament-fore', {
    yPercent: heroOrnamentForeShift,
    ease: 'none',
    scrollTrigger: {
      trigger: hero,
      start: 'top top',
  ...
  // Parallax na foto de perfil: desloca em velocidade propria, camada mais "proxima" da cena.
  // Aplicado na <img> interna (nao no <figure>.logo), que ja tem a animacao floatY
  // continua no mesmo transform e sobrescreveria o parallax do GSAP se fosse no mesmo elemento.
  ...
  // Eco sutil do parallax do hero em #sobre e #tech-content (ornamentos de baixa opacidade).
  gsap.utils.toArray('.panel-ornament').forEach(function (ornament) {
    const panel = ornament.closest('.panel');
    if (!panel) return;
    gsap.to(ornament, {
