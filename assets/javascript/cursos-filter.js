(function(){
  ...
  // Renderiza os cursos a partir de assets/javascript/cursos-data.js.
  // Adicionar um curso novo = adicionar uma linha no array, sem mexer no HTML.
  function renderCursos(){
    const data = window.CGS_CURSOS;
    if (!data) return;

    const lists = [
      { id: 'cursos-tecnologia-list', items: data.tecnologia },
      { id: 'cursos-gerais-list', items: data.gerais }
    ];

    lists.forEach(function(group){
      const ol = document.getElementById(group.id);
      if (!ol || !Array.isArray(group.items)) return;
